import './my_skills.css'
const My_skills = ()=>{


return(<> 
<div id='skill'>
	<h1 id = 'entry' style = {{fontWeight:'bold'}}>My Skills</h1>
	<br/>
	<h2 id = 'section'style = {{fontWeight:'bold'}}>Frontend:</h2>
	<br/>
<div class='row'>
 <div className="col-6 col-md-4 col-lg-3">
<div class="card_" >
<img  id ='js' class ='card-img-top'src ='https://cdn.iconscout.com/icon/free/png-512/free-javascript-1-225993.png?f=avif&w=256'  />
 <div class="card-body">
    <p class="card-text">Javascript</p>
  </div>
</div>
</div>
 <div className="col-6 col-md-4 col-lg-3">
<div class="card_" >
<img   class ='card-img-top'src ='https://cdn.iconscout.com/icon/free/png-512/free-react-1-282599.png?f=avif&w=256'  />
 <div class="card-body">
    <p class="card-text">React</p>
  </div>
</div>

</div>
<div className="col-6 col-md-4 col-lg-3">
<div class="card_" >
<img   class ='card-img-top'src ='https://reactrouter.com/twitterimage.jpg' />
 <div class="card-body">
    <p class="card-text">React Router</p>
  </div>
</div>
</div>
<div className="col-6 col-md-4 col-lg-3">
<div class="card_" >
<img   class ='card-img-top'src ='https://cdn.iconscout.com/icon/free/png-512/free-html5-42-1175210.png?f=avif&w=256' />
 <div class="card-body">
    <p class="card-text">HTML</p>
  </div>
</div>
</div>





</div>

<br/>
<br/>
<br/>
<br/>


<div class='row'>
 <div className="col-6 col-md-4 col-lg-3">
<div class="card_" >
<img   class ='card-img-top'src ='https://cdn.iconscout.com/icon/free/png-512/free-css3-8-1175200.png?f=avif&w=256'  />
 <div class="card-body">
    <p class="card-text">CSS</p>
  </div>
</div>
</div>
 <div className="col-6 col-md-4 col-lg-3">
<div class="card_" >
<img   class ='card-img-top'src ='https://cdn.iconscout.com/icon/free/png-512/free-bootstrap-6-1175203.png?f=avif&w=256'  />
 <div class="card-body">
    <p class="card-text">Bootstrap</p>
  </div>
</div>


</div>





</div>


   <br/> <br/> <br/>
	<br/>
	<h2 id = 'section'style = {{fontWeight:'bold'}}>Backend:</h2>
	<br/>
<div class='row'>
 <div className="col-6 col-md-4 col-lg-3">
<div class="card_" >
<img  class ='card-img-top'src ='https://cdn.iconscout.com/icon/free/png-512/free-c-plus-plus-5285214-4406651.png?f=avif&w=256'  />
 <div class="card-body">
    <p class="card-text">C/C++</p>
  </div>
</div>
</div>
 <div className="col-6 col-md-4 col-lg-3">
<div class="card_" >
<img   class ='card-img-top'src ='https://cdn.iconscout.com/icon/free/png-512/free-java-23-225999.png?f=avif&w=256'  />
 <div class="card-body">
    <p class="card-text">Java</p>
  </div>
</div>

</div>
<div className="col-6 col-md-4 col-lg-3">
<div class="card_" >
<img   class ='card-img-top'src ='data:image/jpeg;base64,' />
 <div class="card-body">
    <p class="card-text">Python</p>
  </div>
</div>
</div>
<div className="col-6 col-md-4 col-lg-3">
<div class="card_" >
<img   class ='card-img-top'src ='https://upload.wikimedia.org/wikipedia/commons/thumb/d/d9/Node.js_logo.svg/2560px-Node.js_logo.svg.png' />
 <div class="card-body">
    <p class="card-text">Node Js</p>
  </div>
</div>
</div>





</div>

<br/>
<br/>
<br/>
<br/>


<div class='row'>
 <div className="col-6 col-md-4 col-lg-3">
<div class="card_" >
<img   class ='card-img-top'src ='https://seeklogo.com/images/M/MySQL-logo-F6FF285A58-seeklogo.com.png'  />
 <div class="card-body">
    <p class="card-text">Mysql</p>
  </div>
</div>
</div>
 <div className="col-6 col-md-4 col-lg-3">
<div class="card_" >
<img   class ='card-img-top'src ='https://hackster.imgix.net/uploads/attachments/1034524/arduino_logo_fwx2rSB3rG.png?auto=compress%2Cformat&w=255&h=170&fit=fill&bg=fff&dpr=2.625'  />
 <div class="card-body">
    <p class="card-text">Arduino Cloud</p>
  </div>
</div>


</div>

 <div className="col-6 col-md-4 col-lg-3">
<div class="card_" >
<img   class ='card-img-top'src ='https://webimages.mongodb.com/_com_assets/cms/kuzt9r42or1fxvlq2-Meta_Generic.png'  />
 <div class="card-body">
    <p class="card-text">MongoDB</p>
  </div>
</div>


</div>



</div>
</div>
<br/>
<br/>
<br/>
	</>)
}
export default My_skills;